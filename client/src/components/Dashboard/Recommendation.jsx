...
const RecomModal = ({ setRecomOpen, stock }) => {
  return (
    <motion.div
    ...
    >
      <Container>
        <motion.div animate={{ opacity: 1, y: -20 }}>
          <RecomModalContent setRecomOpen={setRecomOpen} stock={stock} />
        </motion.div>
      </Container>
    </motion.div>
  );
...
const RecomModalContent = ({ setRecomOpen, stock }) => {
  ...
  const sellStock = async (e) => {
    ...
    const url = `http://localhost:5000/api/stock/recommend`;
